...
func TestTargets(t *testing.T) {
	...
	t.Run("scan target queue stream output", func(t *testing.T) {
		hosts := []string{
			"https://yandex.ru",
			"https://212.122.32.36/path",
			"212.122.32.12/32",
		...
		go queue.Output(outputChan)

		var t_ = make([]string, 0)

		for {
			target, ok := <-outputChan
			if ok {
				t_ = append(t_, target)
			} else {
		...
		require.Len(t, t_, 6)
		require.Equal(t, t_[0], "yandex.ru")
		require.Equal(t, t_[1], "https://yandex.ru")
		require.Equal(t, t_[2], "212.122.32.36")
		require.Equal(t, t_[3], "https://212.122.32.36/path")
		require.Equal(t, t_[4], "212.122.32.12")
		require.Equal(t, t_[5], "ya.ru")
	})

	t.Run("scan target queue stream output with subnet ignore checks, subnet 30", func(t *testing.T) {
		hosts := []string{
			"212.122.32.0/30",
		}
	...
	t.Run("scan target queue stream output with subnet ignore checks, subnet 24", func(t *testing.T) {
		hosts := []string{
			"212.122.32.0/24",
		}
		...
		require.Len(t, t_, 255)
	})

	t.Run("scan target queue stream output with subnet unmasking, subnet 16", func(t *testing.T) {
		hosts := []string{
			"212.132.0.0/16",
		}
		...
		require.Len(t, t_, 65535)
	})

	t.Run("scan target queue stream output with subnet unmasking, subnet 8", func(t *testing.T) {
		hosts := []string{
			"212.0.0.0/8",
		}
